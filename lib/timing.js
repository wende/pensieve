...
const {
  streamValues
} = require('stream-json/streamers/StreamValues');
...
module.exports.getTimeseries = cb => {
  let allFiles = fs.readdirSync(TIMING_FOLDER).filter(name => {
    return name.endsWith(".gz");
  }).map(sourceFile => {
    const pipeline = Chain.chain([fs.createReadStream(TIMING_FOLDER + sourceFile), zlib.createGunzip(), parser(), streamValues()]);
